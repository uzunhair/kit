<?php
    $this->addJS('templates/kit/js/content-bind.js');
    $ids = array();
?>
		...
		<?php foreach($items as $item) { ?>
			<li data-id="<?php echo $item['id']; ?>">
                ...
                <?php if ($is_allowed_to_bind) { ?>
                    <a class="delete" href="#" data-toggle="tooltip" title="<?php echo LANG_DELETE; ?>"><i class="fa fa-trash-o"></i></a>
                <?php } ?>
			</li>
		<?php $ids[] = $item['id']; } ?>
	<?php } ?>
...
<?php if ($is_allowed_to_bind) { ?>
	...
	<a class="add btn btn-secondary" href="<?php echo $url; ?>?input_action=<?php echo $input_action; ?>"><?php echo ($input_action === 'bind' ? LANG_ADD : LANG_SELECT); ?></a>
<?php } ?>
            ...
            icms.contentBind.start({
				...
				callback: function(items){
					...
					$.each(items, function(id, title){
                        ...
                        var $deleteBtn = $('<a href="#" class="ml-1"><i class="fa fa-trash-o"></i></a>').appendTo($item);
